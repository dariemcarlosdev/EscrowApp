...
| Task Category | Agent Type | Default Model | Token Budget | Parallelizable | Example |
|---|---|---|---|---|---|
| Read-only codebase research | `explore` | Haiku / mini / flash | ~20K | ✅ Yes (up to 5) | "Find all usages of OrderService" |
| Build, test, lint, install | `task` | Haiku / mini / flash | ~15K | ⚠️ Caution (side effects) | "Run dotnet test" |
| Complex multi-step implementation | `general-purpose` | Sonnet / GPT-5.x | ~50K | ❌ Serialize | "Implement the release handler" |
| Plan/code review, validation | `critic` | Sonnet / GPT-5.x | ~30K | ❌ Serialize | "Review my implementation plan" |
| Codebase exploration (many modules) | `explore` × N | Haiku / mini / flash | ~20K each | ✅ Yes | "Analyze 5 services in parallel" |
| Security audit | `general-purpose` | Sonnet → Opus (fintech) | ~50K | ❌ Serialize | "Audit payment handler" |
| Architecture review | `general-purpose` | Opus (quality-first) | ~50K | ❌ Serialize | "Review Clean Architecture" |

> **Model selection:** Use the Model Router skill (`.github/skills/ai/model-router/SKILL.md`) or `suggest_model` extension tool for intelligent model selection beyond these defaults.

...
| Agent Type | Default Model | Typical Prompt | Typical Response | Total Per Agent | Cost Tier |
|---|---|---|---|---|---|
| `explore` | Haiku / mini / flash | ~5K input | ~15K output | ~20K total | Low ($) |
| `task` | Haiku / mini / flash | ~3K input | ~12K output | ~15K total | Low ($) |
| `general-purpose` | Sonnet / GPT-5.x | ~15K input | ~35K output | ~50K total | Medium ($$) |
| `critic` | Sonnet / GPT-5.x | ~10K input | ~20K output | ~30K total | Medium ($$) |
| `general-purpose` (Opus override) | Opus | ~15K input | ~35K output | ~50K total | High ($$$) |

> **Model routing:** Default models are starting points. Use the Model Router skill (`.github/skills/ai/model-router/SKILL.md`) to select optimal models based on task category, complexity, and budget mode.

## Cost-Per-Model Reference

| Model Tier | Examples | Input ($/M) | Output ($/M) | Use For |
|---|---|---|---|---|
| Premium | claude-opus-4.6, claude-opus-4.5 | $5.00 | $25.00 | Architecture review, complex debugging, security-critical |
| Standard | claude-sonnet-4.x, gpt-5.x, gemini-pro | $1.25–$3.00 | $10–$15 | Most development tasks |
| Fast/Cheap | claude-haiku, gpt-mini, gpt-4.1, gemini-flash | $0.25–$2.00 | $2–$8 | Exploration, builds, parallel fleets |

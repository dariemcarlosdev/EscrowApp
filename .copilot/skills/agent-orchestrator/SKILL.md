...
### Step 2 — Plan the Fleet and Select Models

Determine agent count, types, **model assignments**, and token budget allocation.
...
2. **Select optimal model** per agent — Use the **Model Router** skill:
   - Classify each work unit's task category (12 categories)
   - Determine complexity (S/M/L/XL)
   - Apply constraints (budget mode, parallel fleet, domain sensitivity)
   - Score candidate models and pick the best fit
   - If `suggest_model` extension tool is available, use it for automated scoring
   - Otherwise, read `.github/skills/ai/model-router/SKILL.md` and follow Steps 1-3
   - **Quick defaults:** explore → Haiku/mini/flash, task → Haiku/mini, general-purpose → Sonnet/GPT-5.x, critic → Sonnet
3. **Allocate token budget** — Total budget = context window × 0.8. Divide across agents by complexity.
4. **Set blast radius** — Define what each agent is allowed to touch (files, directories, commands).
5. **Plan error recovery** — For each agent: retry policy, fallback strategy, manual takeover threshold.
...
| Agent Type | Default Model Tier | When to Override |
|---|---|---|
| explore | Fast/Cheap (Haiku, mini, flash) | Complex cross-cutting analysis → Standard |
| task | Fast/Cheap (Haiku, mini, flash) | Rarely — command execution is model-agnostic |
| general-purpose | Standard (Sonnet, GPT-5.x) | Security/fintech → Premium (Opus) |
| critic | Standard (Sonnet, GPT-5.x) | Architecture review → Premium (Opus) |

**✅ Checkpoint: Agent types assigned, models selected per agent, token budget allocated (sum ≤ 80% of total), blast radius defined, error recovery planned.**

...
#
# Usage:
#   .\notification                  # Normal run (prompts for email credentials if needed)
#   .\notification -SkipEmail       # Skip email notifications entirely
#   .\notification -NoPrompt        # Don't prompt for credentials (skip email if not stored)
...
if (Test-Path $configPath) {
    ...
    $defaultConfig | ConvertTo-Json -Depth 3 | > $configPath
...
if ($config.rateLimit.enabled) {
    ...
    if (Test-Path $lastTimePath) {
        try {
            $lastSent = cat $lastTimePath -Raw | Get-Date
            ...
            if (((Get-Date) - $lastSent).TotalSeconds -lt $interval) {
                ...
                echo "Rate limit active. Wait $remaining seconds." -ForegroundColor Yellow
                $rateLimited = $true
            }
        } catch {
...
if ($rateLimited) {
    if ($config.console.enabled) {
        echo "$message (rate limited)" -ForegroundColor Yellow
    }
...
if ($config.rateLimit.enabled) {
    $lastTimePath = $config.rateLimit.lastNotificationFile
    if (-not $lastTimePath) { $lastTimePath = ".claude/hooks/.rate-limit-timestamp" }
    $timestamp | > -Path $lastTimePath -Encoding UTF8
}
...
# -------------------------------------------------
if ($config.console.enabled) {
    echo "[BELL] $message" -ForegroundColor Cyan
}
...
if ($config.slack.enabled -and $config.slack.webhookUrl) {
    try {
        ...
        echo "[OK] Slack notification sent" -ForegroundColor Green
    } catch {
...
# -------------------------------------------------
if ($SkipEmail) {
    echo "[INFO] Email notification skipped (-SkipEmail)" -ForegroundColor DarkGray
    ...
    if ($accounts.Count -eq 0) {
        echo "[INFO] Email notification: no enabled accounts with valid config" -ForegroundColor DarkGray
    } else {
...
"@
                ...
                if (Test-Path $credPath) {
                    try {
                        ...
                        echo "[INFO] Using stored credentials for $($account.from)" -ForegroundColor DarkGray
                    } catch {
                ...
                if ((-not (Test-Path $credPath)) -or $null -eq $credential) {
                    if ($NoPrompt) {
                        echo "[INFO] No stored credentials for $($account.from) and -NoPrompt specified. Skipping email." -ForegroundColor DarkGray
                        continue
                    }

                    echo "[INFO] Enter SMTP credentials for $($account.from) on $($account.smtpServer)" -ForegroundColor Yellow
                    try {
                        $credential = Get-Credential -UserName $account.from -Message "Enter password for $($account.from)"
                        if ($credential) {
                            $credential | Export-CliXml -Path $credPath
                            echo "[OK] Credentials saved (encrypted)" -ForegroundColor Green
                        } else {
                ...
                echo "[OK] Email sent via $($account.from) to $($account.to)" -ForegroundColor Green
                break
            } catch {
...
if ($config.teams.enabled -and $config.teams.webhookUrl) {
    try {
        ...
        echo "[OK] Teams notification sent" -ForegroundColor Green
    } catch {

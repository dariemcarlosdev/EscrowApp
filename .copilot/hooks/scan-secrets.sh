...
param(
    ...
    [string]$AllowlistRaw = $SECRETS_ALLOWLIST
)

if ($SKIP_SECRETS_SCAN -eq "true") {
...
# Report
# ---------------------------------------------------------------------------
echo "🔍 Scanned $($files.Count) staged file(s) for secrets..."

if ($findings.Count -gt 0) {
    echo ""
    echo "⚠️  Found $($findings.Count) potential secret(s):" -ForegroundColor Yellow
    echo ""
    echo ("  {0,-45} {1,-6} {2,-28} {3}" -f "FILE", "LINE", "PATTERN", "SEVERITY")
    echo ("  {0,-45} {1,-6} {2,-28} {3}" -f "----", "----", "-------", "--------")
    ...
    foreach ($f in $findings) {
        ...
        echo ("  {0,-45} {1,-6} {2,-28} {3}" -f $f.File, $f.Line, $f.Pattern, $f.Severity) -ForegroundColor $color
    }
    ...
    if ($Mode -eq "block") {
        ...
        if ($criticalOrHigh.Count -gt 0) {
            echo "🚫 Commit blocked: $($criticalOrHigh.Count) critical/high finding(s). Remove secrets before committing." -ForegroundColor Red
            echo "   Set SCAN_MODE=warn to log without blocking, or add patterns to SECRETS_ALLOWLIST." -ForegroundColor DarkGray
            exit 1
        } else {
            echo "💡 Medium-severity findings detected (not blocking). Review recommended." -ForegroundColor Yellow
        }
    } else {
        echo "💡 Review the findings above. Set SCAN_MODE=block to prevent commits with secrets." -ForegroundColor Yellow
    }
} else {
    echo "✅ No secrets detected in $($files.Count) scanned file(s)" -ForegroundColor Green
}
